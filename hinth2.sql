...
select fh.path
      ...
      ,regexp_replace(regexp_replace(version,'(\d+)','0000\1'),'\d*(\d{2})','\1') ver1
...
where hi.sql_feature(+) = fh.sql_feature
  ...
  and ('&_min_ver'   is null or regexp_replace(regexp_replace(version,'(\d+)','0000\1'),'\d*(\d{2})','\1')>= regexp_replace(regexp_replace('&_min_ver','(\d+)','0000\1'),'\d*(\d{2})','\1'))
