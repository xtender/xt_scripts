...
begin
   select 
       wm_concat('max(decode(child_number,'|| s.child_number||',value)) as child_'||child_number)
     into v_childs
   from v$sql s
   ...
   v_sql := '
      ...
      select sql_id,id,opt_name,'||v_childs||'
      from 
        (select * from x where cnt>1) e
      group by sql_id,id,opt_name
      order by id';
