...
accept v_low - 
		prompt 'Enter value for low value: ';

accept v_high - 
		prompt 'Enter value for high value: ';
...
BEGIN
   ...
   IF rec_srec.epc = 2 THEN
     ...
     d_low := raw_to_num(r_low);
     d_high:= raw_to_num(r_high);
     
     dbms_output.put_line('Old low  value: '|| to_char(d_low ));
     dbms_output.put_line('Old high value: '|| to_char(d_high));
     
     d_low  := &v_low ;
     d_high := &v_high;
          
     vals := DBMS_STATS.numarray(d_low, d_high);
     ...
     DBMS_STATS.PREPARE_COLUMN_VALUES(rec_srec, vals);
     ...
     dbms_output.put_line('New low  value: '|| to_char(d_low ));
     dbms_output.put_line('New high value: '|| to_char(d_high));
   ELSE
