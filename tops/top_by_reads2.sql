...
begin
   ...
   close c_stats;
   select value into v_sysstat1 from v$sysstat where statistic#=p_stat;
   ...
   close c_stats;
   select value into v_sysstat2 from v$sysstat where statistic#=p_stat;
   
   v_delta := v_sysstat2-v_sysstat1;
   ...
   loop
      dbms_output.put_line(
         utl_lms.format_message(
           ...
           , lpad(r.event    ||' ',15,' ')
           , lpad(r.status   ||' ',15,' ')
           , lpad(to_char( r.delta *100 /v_delta,'90d990'),15,' ')
         ));
   end loop;
   dbms_output.put_line(lpad('-',152,'-'));
end;
